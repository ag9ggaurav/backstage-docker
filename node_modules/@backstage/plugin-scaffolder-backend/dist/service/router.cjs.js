...
function formatSecretsValidationErrors(result) {
  return result.errors.map((err) => {
    const property = err.property.replace(/^instance/, "secrets");
    const secretName = err.argument;
    const message = err.name === "required" ? `secrets.${secretName} is required` : `${property} ${err.message}`;
    return {
      ...err,
      property,
      message,
...
async function validateSecrets(options) {
  ...
  const result = jsonschema.validate(secrets, template.spec.secrets.schema);
  if (result.valid) {
    return true;
  }
  await auditorEvent?.fail({
    error: new errors.InputError("Secrets validation failed")
  });
  res.status(400).json({
...
async function createRouter(options) {
  ...
  permissionsRegistry.addResourceType({
    resourceRef: alpha$1.scaffolderTemplatePermissionResourceRef,
    permissions: alpha.scaffolderTemplatePermissions,
    rules: templateRules
  });
  permissionsRegistry.addResourceType({
    resourceRef: alpha$1.scaffolderActionPermissionResourceRef,
    permissions: alpha.scaffolderActionPermissions,
    rules: actionRules
  });
  permissionsRegistry.addResourceType({
    resourceRef: alpha$1.scaffolderTaskPermissionResourceRef,
  ...
  permissionsRegistry.addPermissions([
    alpha.taskCreatePermission,
    alpha.templateManagementPermission
  ]);
  router$1.get(
    ...
    try {
      ...
      if (task.spec.templateInfo?.entityRef) {
        const templateEntityRef = catalogModel.parseEntityRef(
  ...
  router$1.get("/v2/tasks/:taskId/events", async (req, res) => {
    ...
    try {
      ...
      const bodySchema = v3.z.object({
        template: v3.z.unknown(),
        values: v3.z.record(v3.z.unknown()),
        secrets: v3.z.record(v3.z.string()).optional(),
        directoryContents: v3.z.array(
          v3.z.object({ path: v3.z.string(), base64Content: v3.z.string() })
        )
      });
      ...
      const templateInfo = {
        ...
        baseUrl: node_url.pathToFileURL(
