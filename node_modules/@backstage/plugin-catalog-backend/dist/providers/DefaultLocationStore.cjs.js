  ...
  async updateLocation(id, location) {
    ...
    if (this.db.client.config.client.includes("mysql")) {
      await this.db.transaction(async (tx) => {
        [row] = await tx("locations").where({ id }).select();
        if (!row) {
          return;
        }
        const [conflict] = await tx("locations").where({ type: location.type, target: location.target }).whereNot({ id }).select();
        if (conflict) {
          throw new errors.ConflictError(
            `Location ${location.type}:${location.target} already exists`
          );
        }
        await tx("locations").where({ id }).update({ type: location.type, target: location.target });
      ...
      await this.db.transaction(async (tx) => {
        ...
        [row] = await tx("locations").where({ id }).update({ type: location.type, target: location.target }).returning("*");
      });
    }
    if (!row) {
      throw new errors.NotFoundError(`Found no location with ID ${id}`);
