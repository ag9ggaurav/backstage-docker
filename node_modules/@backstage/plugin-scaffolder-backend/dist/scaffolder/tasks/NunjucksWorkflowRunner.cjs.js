...
class NunjucksWorkflowRunner {
  ...
  async executeStep(task, step, context, renderTemplate, taskTrack, workspacePath, decision) {
    ...
    try {
      if (step.if === false || typeof step.if === "string" && step.each === void 0 && !helper.isTruthy(this.render(step.if, context, renderTemplate))) {
      ...
      const iterations = (resolvedEach ? Object.entries(resolvedEach).map(([key, value]) => ({
        ...
        const shouldRun = !("each" in i) || !step.if || helper.isTruthy(this.render(step.if, fullContext, renderTemplate));
        return {
          ...i,
          shouldRun,
          // Secrets are only passed when templating the input to actions for security reasons
          input: step.input ? this.render(step.input, fullContext, renderTemplate) : {}
        };
      });
  ...
  async execute(task) {
    ...
    try {
      ...
      if (Array.isArray(output?.links)) {
        output.links = helper.filterConditionalItems(output.links);
      }
      if (Array.isArray(output?.text)) {
        output.text = helper.filterConditionalItems(output.text);
      }
