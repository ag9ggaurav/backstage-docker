...
function computeLocationEntityRef(type, target) {
  return catalogModel.stringifyEntityRef({
    kind: "Location",
    namespace: "default",
...
function locationSpecToLocationEntity(opts) {
  ...
  const name = opts.locationEntityRef ? catalogModel.parseEntityRef(opts.locationEntityRef).name : locationSpecToMetadataName(location);
  let ownLocation;
  let originLocation;
  if (parentEntity) {
...
exports.computeLocationEntityRef = computeLocationEntityRef;
