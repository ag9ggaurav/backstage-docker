...
var mergeJsonSchemas = require('./jsonSchema/mergeJsonSchemas.cjs.js');
var util = require('./operations/util.cjs.js');
...
function sortOps(input) {
  return lodash__default.default.sortBy(input, (op) => util.ops[op.op].order);
}
function applyDeclareKind(kinds, op) {
  if (kinds.has(op.kind)) {
    throw new errors.InputError(`Kind "${op.kind}" is declared more than once`);
  }
  kinds.set(op.kind, {
    group: op.group,
...
function applyDeclareRelation(relations, op) {
  ...
  if (existing) {
    ...
    relations.set(op.type, {
      fromKinds: /* @__PURE__ */ new Set([op.fromKind]),
      toKinds: /* @__PURE__ */ new Set([op.toKind]),
      description: op.properties.description,
      forward: {
        type: op.type,
...
function applyUpdateKind(kinds, op) {
  const kind = kinds.get(op.kind);
  if (!kind) {
    throw new errors.InputError(`Cannot update unknown kind "${op.kind}"`);
  }
...
function applyUpdateKindVersion(kinds, op) {
  ...
  if (op.properties.schema !== void 0) {
    specType.jsonSchema = mergeJsonSchemas.mergeJsonSchemas(
      specType.jsonSchema,
...
function applyUpdateRelation(relations, op) {
  ...
  if (!relation) {
    throw new errors.InputError(`Cannot update undeclared relation "${op.type}"`);
  }
...
function applyRemoveKind(kinds, op) {
  if (!kinds.has(op.kind)) {
    throw new errors.InputError(`Cannot remove unknown kind "${op.kind}"`);
  }
  kinds.delete(op.kind);
}
function applyDeclareAnnotation(annotations, op) {
  if (annotations.has(op.name)) {
    throw new errors.InputError(`Annotation "${op.name}" is declared more than once`);
  }
...
function applyDeclareLabel(labels, op) {
  if (labels.has(op.name)) {
    throw new errors.InputError(`Label "${op.name}" is declared more than once`);
  }
  labels.set(op.name, {
    title: op.properties.title,
    description: op.properties.description,
...
function applyDeclareTag(tags, op) {
  if (tags.has(op.name)) {
    throw new errors.InputError(`Tag "${op.name}" is declared more than once`);
  }
  tags.set(op.name, {
    title: op.properties.title,
    description: op.properties.description
...
function applyUpdateAnnotation(annotations, op) {
  ...
  if (!annotation) {
    throw new errors.InputError(`Cannot update undeclared annotation "${op.name}"`);
  }
  if (op.properties.title !== void 0) {
...
function applyUpdateLabel(labels, op) {
  ...
  if (!label) {
    throw new errors.InputError(`Cannot update undeclared label "${op.name}"`);
  }
  if (op.properties.title !== void 0) {
...
function applyUpdateTag(tags, op) {
  ...
  if (!tag) {
    throw new errors.InputError(`Cannot update undeclared tag "${op.name}"`);
  }
  if (op.properties.title !== void 0) {
...
function applyRemoveAnnotation(annotations, op) {
  if (!annotations.has(op.name)) {
    throw new errors.InputError(`Cannot remove unknown annotation "${op.name}"`);
  }
  annotations.delete(op.name);
}
function applyRemoveLabel(labels, op) {
  if (!labels.has(op.name)) {
    throw new errors.InputError(`Cannot remove unknown label "${op.name}"`);
  }
  labels.delete(op.name);
}
function applyRemoveTag(tags, op) {
  if (!tags.has(op.name)) {
    throw new errors.InputError(`Cannot remove unknown tag "${op.name}"`);
  }
  tags.delete(op.name);
}
function buildFullSchema(options) {
  ...
  return mergeJsonSchemas.mergeJsonSchemas(options.kindSchema, generatedSchema);
}
function compileCatalogModel(inputs) {
  let allOps = [];
  for (const input of inputs) {
    const internal = types.OpaqueCatalogModelLayer.toInternal(input);
  ...
  for (const op of sortedOps) {
    switch (op.op) {
      ...
      case "removeKind.v1":
        ...
        break;
      default:
        throw new errors.InputError(`Unknown op type "${op.op}"`);
    }
  }
  ...
  for (const kindName of kinds.keys()) {
    compiledRelations.set(
      kindName,
      [...relations.values()].filter((r) => r.fromKinds.has(kindName)).map((r) => {
...
exports.compileCatalogModel = compileCatalogModel;
//# sourceMappingURL=compileCatalogModel.cjs.js.map
